...
public class AdminMemberExpandableListAdapter extends BaseExpandableListAdapter{
	...
	public AdminMemberExpandableListAdapter(Context context,List<SessionInfo> get_sessions){
		...
		for(int deptPosition=0;deptPosition<deptCount;deptPosition++){
			SessionInfo SessionInfo=get_sessions.get(deptPosition);
			List<PersonInfo> PersonInfos=SessionInfo.getPersonInfos();
		}
	}
	@Override
	public Object getChild(int deptPosition, int perPosition) {
		final SessionInfo SessionInfo=deptList.get(deptPosition);
		if(SessionInfo==null||SessionInfo.getPersonInfos()==null||SessionInfo.getPersonInfos().isEmpty()){
			return null;
			
		}
		return SessionInfo.getPersonInfos().get(perPosition);
	}
	public String getPersonName(int deptPosition, int perPosition) {
		...
		return SessionInfo.getPersonInfos().get(perPosition).getName();
	}
	@Override
	...
	public View getChildView(final int deptPosition,final int perPosition,boolean isLastPerson,View convertView,final ViewGroup parent) {
		...
		final PersonInfo PersonInfo=(PersonInfo)getChild(deptPosition,perPosition);
		...
		viewHolder.textViewPersonName.setText(PersonInfo.getName());
		viewHolder.imageViewPower.setImageResource(PersonInfo.getImage_power());
...
//		});
	...
	public int getChildrenCount(int deptPosition) {
		// TODO Auto-generated method stub
		final SessionInfo SessionInfo=deptList.get(deptPosition);
		if(SessionInfo==null||SessionInfo.getPersonInfos()==null||
				SessionInfo.getPersonInfos().isEmpty()){
			return 0;
		}
		return SessionInfo.getPersonInfos().size();
	}
	...
	public View getGroupView(int deptPosition, boolean isExpanded, View convertView, ViewGroup parent) {
		// TODO Auto-generated method stub
		try{
			SessionInfo SessionInfo=deptList.get(deptPosition);
			...
			viewHolder.textViewDeptName.setText(SessionInfo.getName());
		}
		catch(Exception e){
			e.printStackTrace();
